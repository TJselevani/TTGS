...
def generateTimetable():
    ...
    print("Number of units:", len(units))  # Debug
    print("Number of lecturers:", len(lecturers))  # Debug
    print("Number of classrooms:", len(classrooms))  # Debug
    ...
    assignments = LpVariable.dicts(
        "Assignment",
        (
            (semester, unit, classroom, day, period)
    ...
    prob += lpSum(
        [
            assignments[(semester, unit, classroom, day, period)]
            for semester in semesters
            for unit in units
            for classroom in classrooms
            for day in days
                ...
                prob += (
                    lpSum(
                        [
                            ...
                            for semester in semesters
                            for unit in units
                            if unit.lecturer == lecturer.lecturerName  # VERY IMPORTANT
                        ]
                    )
                    <= 1
                )
                ...
                prob += (
                    lpSum(
                        [
                            ...
                            in unit.classroom  # ALSO VERY IMPORTANT
                        ]
                    )
                    <= 1000
                )
        ...
        for unit in units:
            for classroom in classrooms:
                for day in days:
                    for period in periods:
                        if (
                            value(assignments[(semester, unit, classroom, day, period)])
                            == 1
                        ):
                            unitss = [
                                *unitss,
