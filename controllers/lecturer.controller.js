...
const lecturerPage = async (req, res) => {
    ...
    res.render('lecturer', { docs: lecUnits.unit, title: 'Lecturer', name: lec, message: '',  timetableData: filteredUnits }); ///tTable ? tTable.unitss : [] 
}
...
const selectUnitsPage = async (req, res) => {
    ...
    const filteredUnits = tTable.map(doc => ({...doc.toObject(), unitss: doc.unitss.filter(unit => unit.unitLecturer === lec) }));
    res.render('select', {unitInfo: result , title: 'Lecturer-select', name: lec, timetableData: filteredUnits});//tTable ? tTable.unitss : [] 
}
...
const selectUnits = async (req, res) => {
    ...
    try 
    {
        ...
        const tTable = await TimetableMDL.find()
        const filteredUnits = tTable.map(doc => ({...doc.toObject(), unitss: doc.unitss.filter(unit => unit.unitLecturer === lec) }));
        res.redirect('/lecturer', 301, {title: 'Lecturer', name: lec,  timetableData: filteredUnits})// tTable ? tTable.unitss : []
    } catch (error) {
        console.log(error);
